...
namespace Game
{
    public class PlayerShip : GameObject
    {
        ...
        {
            graphics.DrawImage(PlayerShipImage, Bounds);
        }
        
        private Image LoadImage()
        {
            Image[] ships = Spritesheet.Load(@"Resources\shipsheetparts.png", new Size(200, 200)); //eso es raro
            foreach (Image img in ships) //este for no tiene sentido solo deberia rotar la suya, el for lo podria borrar por completo creo
            {
                img.RotateFlip(RotateFlipType.Rotate270FlipNone);
            }
            Image result = ships[shipIndex]; //es el 33, esta hardcodeado
